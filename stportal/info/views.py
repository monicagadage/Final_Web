...
@login_required()
def messageStudent(request, stud_id ):
    teacher_list = Teacher.objects.all()
    return render(request, 'info/t_list.html', {'teacher_list' : teacher_list, 'stud_id' : stud_id })

@login_required()
def messageTeacher(request, teacher_id ):
    student_list = Student.objects.all()
    return render(request, 'info/s_list.html', {'student_list' : student_list , 'teacher_id' : teacher_id })

@login_required()
def room(request, stud_name , teacher_id ,choice):
    if(choice == 1):
        username = teacher_id
    else:
        username = stud_name
    # stud = get_object_or_404(Student, name=stud_name)
    return render(request, 'info/room.html', {'room_name' : stud_name , 'username': username})
